...
async def jina_results_to_leads(state: DotDBState, config: Optional[RunnableConfig] = None) -> Dict:
    ...
    for it in filtered:
        ...
        few_shot = (
            ...
            "}\n\n"
            "Skip example (parked/non-business):\n"
            "{}\n\n"
        )
        attempt_prompt = (
            "You are a lead qualification analyst. From the following website details, extract a single high-quality B2B lead\n"
            "ONLY if it appears to be an actual operating business (not parked, placeholder, personal blog, directory, or content farm).\n"
            "Use title, description, and especially content to decide. If parked/inactive/irrelevant, return an empty JSON object {}.\n\n"
            "Use the following classification guidance to judge relevance and assign tier appropriately.\n"
            f"CLASSIFICATION GUIDANCE:\n{cls_out}\n\n"
            "Return a JSON object with EXACT keys: website, detailed_summary, rationale, tier, meta_data.\n"
